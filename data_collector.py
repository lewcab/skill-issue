...
from constants import HISTORY_LENGTH, PLAYERS_PER_TEAM, OUTPUT_NAME, ROLE_TO_NUM
...
def main():
    ...
    for t in tournaments: print(f"\t- {t}")
...
def get_tournaments(client: EsportsClient, region: str = None, limit: int = 500) -> list:
    """
...
def get_matches(client: EsportsClient, tournaments: list[str]) -> int:
    ...
    for tournament in tournaments:
        ...
        if len(matches) == 0:
            ...
            print(f"+\n+ Found {len(matches)} matches for tournament: {tournament}\n+")
        for m in matches:
            team_1 = m['Team1']
            team_2 = m['Team2']
        ...
        count += len(matches)
        print(f"+\n+ Finished processing for tournament: {tournament}\n+\n")
        write_to_csv(data)
...
def get_stats(
    ...
    for stat, val in team_1_stats.items():
        match_data[f'1_0_{stat}'] = val
    for stat, val in team_2_stats.items():
        match_data[f'2_0_{stat}'] = val
    for stat, val in team_1_player_stats.items():
        match_data[f'1_{stat}'] = val
    for stat, val in team_2_player_stats.items():
        match_data[f'2_{stat}'] = val

    return match_data


def get_tournament_matches(client: EsportsClient, tournament: str) -> list:
    """
...
def get_team_stats(client: EsportsClient, team_name: str, match_datetime: str) -> dict:
    ...
    for i, match in enumerate(data):
        ...
        try:
            ...
            stats['gold_lead'] = stats.get('gold_lead', 0) + (int(match.get(f"{team}Gold")) - int(match.get(f"{enemy}Gold")))
            stats['kills'] = stats.get('kills', 0) + int(match.get(f"{team}Kills") or 0)
            stats['kills_lead'] = stats.get('kills_lead', 0) + (int(match.get(f"{team}Kills") or 0) - int(match.get(f"{enemy}Kills") or 0))
            stats['dragons'] = stats.get('dragons', 0) + int(match.get(f"{team}Dragons") or 0)
            stats['dragons_lead'] = stats.get('dragons_lead', 0) + (int(match.get(f"{team}Dragons") or 0) - int(match.get(f"{enemy}Dragons") or 0))
            stats['barons'] = stats.get('barons', 0) + int(match.get(f"{team}Barons") or 0)
            stats['barons_lead'] = stats.get('barons_lead', 0) + (int(match.get(f"{team}Barons") or 0) - int(match.get(f"{enemy}Barons") or 0))
            stats['towers'] = stats.get('towers', 0) + int(match.get(f"{team}Towers") or 0)
            stats['towers_lead'] = stats.get('towers_lead', 0) + (int(match.get(f"{team}Towers") or 0) - int(match.get(f"{enemy}Towers") or 0))
            stats['inhibitors'] = stats.get('inhibitors', 0) + int(match.get(f"{team}Inhibitors") or 0)
            stats['inhibitors_lead'] = stats.get('inhibitors_lead', 0) + (int(match.get(f"{team}Inhibitors") or 0) - int(match.get(f"{enemy}Inhibitors") or 0))
            stats['heralds'] = stats.get('heralds', 0) + int(match.get(f"{team}RiftHeralds") or 0)
            stats['heralds_lead'] = stats.get('heralds_lead', 0) + (int(match.get(f"{team}RiftHeralds") or 0) - int(match.get(f"{enemy}RiftHeralds") or 0))
            stats['grubs'] = stats.get('grubs', 0) + int(match.get(f"{team}VoidGrubs") or 0)
            stats['grubs_lead'] = stats.get('grubs_lead', 0) + (int(match.get(f"{team}VoidGrubs") or 0) - int(match.get(f"{enemy}VoidGrubs") or 0))
...
def get_player_stats(client: EsportsClient, team_name: str, match_datetime: str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")) -> dict:
    ...
    for match in data:
        ...
        try:
            ...
            stats[f"{role_num}_count"] = stats.get(f"{role_num}_count", 0) + 1
            stats[f"{role_num}_kills"] = stats.get(f"{role_num}_kills", 0) + int(match.get("Kills") or 0)
            stats[f"{role_num}_deaths"] = stats.get(f"{role_num}_deaths", 0) + int(match.get("Deaths") or 0)
            stats[f"{role_num}_assists"] = stats.get(f"{role_num}_assists", 0) + int(match.get("Assists") or 0)
            stats[f"{role_num}_gold"] = stats.get(f"{role_num}_gold", 0) + int(match.get("Gold"))
            stats[f"{role_num}_cs"] = stats.get(f"{role_num}_cs", 0) + int(match.get("CS") or 0)
            stats[f"{role_num}_dmg"] = stats.get(f"{role_num}_dmg", 0) + int(match.get("DamageToChampions"))
        except Exception as e:
    ...
    for k, v in stats.items():
        ...
        if stat == 'count' and v != HISTORY_LENGTH:
            ...
            return {}
    for role_num in ROLE_TO_NUM.values():
        del stats[f"{role_num}_count"]
    ...
    with open(OUTPUT_NAME, 'a', newline='') as csvfile:
        ...
        for row in data:
            try:
                ...
                print(f"Error writing row to CSV: {row}\n{e}")

    print(f"{count} rows written to {OUTPUT_NAME} successfully.")
